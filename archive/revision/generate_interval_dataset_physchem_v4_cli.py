...
# -*- coding: utf-8 -*-
"""
generate_interval_dataset_physchem_v4_cli.py
...
예시:
  python generate_interval_dataset_physchem_v4_cli.py \
    --script scripts/data/generate_interval_dataset_physchem_v4.py \
    --config configs/dataset_interval_physchem_v4_deg25.yaml \
    --degree 30

핵심:
- 원본 생성기 로직은 그대로 둠
- 임시 YAML을 만들어 dataset.degree / dataset.out_dir / dataset.n_total / dataset.seed 등을 override
- 이후 원본 스크립트를 DATASET_CFG, OUT_DIR 환경변수로 실행
"""
...
def infer_out_dir(cfg: dict, degree: int, user_out_dir: str | None) -> str:
    ...
    out_dir = dataset.get("out_dir", "./taylor_data_physchem_v4_interval_deg25")
...
def parse_args():
    p = argparse.ArgumentParser(description="Interval dataset generator CLI wrapper")
    p.add_argument("--script", required=True,
                   help="원본 생성기 경로 (예: scripts/data/generate_interval_dataset_physchem_v4.py)")
...
def main():
    ...
    print("[INFO] Running interval dataset generator")
