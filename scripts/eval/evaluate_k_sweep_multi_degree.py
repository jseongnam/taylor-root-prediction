...
class ANNRootRegressor(nn.Module):
    def __init__(self, in_dim: int, num_roots: int, arch_cfg: dict):
        ...
        if isinstance(layers, str) and layers.lower() == "auto":
            hlist = [hidden_dim, hidden_dim, hidden_dim] if in_dim <= 64 else [hidden_dim] * 4 if in_dim <= 256 else [hidden_dim] * 5
...
def _resolve_optional_json(base_path: Path, path_str: str, repo_root: Path):
    ...
    if p.is_absolute() and p.exists():
        return p
    for cand in ((base_path.parent / p).resolve(), (repo_root / p).resolve()):
        if cand.exists():
            return cand
    return None
    ...
    if isinstance(obj, dict):
        for key in ("model", "state_dict", "model_state"):
            if key in obj and isinstance(obj[key], dict):
                if all(torch.is_tensor(v) for v in obj[key].values()):
                    return dict(obj[key]), key
...
def _build_modern_ann_from_sd(sd: dict, ck: dict, repo_root: Path, ckpt_path: Path, device: torch.device):
    ...
    if isinstance(ck, dict) and "in_dim" in ck:
        in_dim = int(ck["in_dim"])
    elif isinstance(model_cfg, dict):
        inp = model_cfg.get("input", {})
        if isinstance(inp, dict):
            if "dimension" in inp:
                in_dim = int(inp["dimension"])
            elif "order" in inp:
                in_dim = int(inp["order"]) + 1

    if isinstance(ck, dict) and "num_roots" in ck:
        num_roots = int(ck["num_roots"])
    elif isinstance(model_cfg, dict):
        out_cfg = model_cfg.get("output", {})
        if isinstance(out_cfg, dict) and "num_roots" in out_cfg:
            num_roots = int(out_cfg["num_roots"])

    if not arch_cfg:
        hidden_guess = int(sd[first_linear_key].shape[0])
        backbone_linears = [k for k, v in sd.items() if k.startswith("backbone.") and k.endswith(".weight") and torch.is_tensor(v) and v.ndim == 2]
...
def load_backend_ann_mdpi(ckpt_path: Path, device: torch.device, repo_root: Path):
    ...
    if isinstance(ck, dict) and "model" in ck and isinstance(ck["model"], dict) and "in_dim" in ck and "out_dim" in ck:
        ...
        if kind == "shallow":
            model = _build_shallow_ann_from_sd(sd, device=device)
        else:
            model = _build_modern_ann_from_sd(sd, ck if isinstance(ck, dict) else {}, repo_root, ckpt_path, device=device)
    ...
    if ("y_min" in sc) and ("y_max" in sc):
        ...
        print(f"[ann] scaler without y_min/y_max -> output treated as original scale: {spath}")
