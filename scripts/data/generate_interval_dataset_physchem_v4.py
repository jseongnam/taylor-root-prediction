...
# -*- coding: utf-8 -*-
"""
scripts/data/generate_interval_dataset_physchem_v4.py

✅ Transformer interval 학습용 데이터셋 생성기 (physchem_v4_interval)
- 다중근 허용
- 기본 구간 [-root_range, root_range]
- 출력 안정화 필터: |P(x)| 폭발 샘플 제거 + 너무 평평한 샘플 제거
- 저장: coeffs, root0/root1/root2, root_count, roots(padded), template_str, expr_str(optional), norm_scale

✅ argparse 없이 YAML만으로 동작
기본 실행:
  python scripts/data/generate_interval_dataset_physchem_v4.py

환경변수 override:
  DATASET_CFG=configs/dataset_interval_physchem_v4_deg25.yaml
...
def generate_dataset(
    ...
    while len(coeffs_list) < num_samples and tries < max_tries:
        ...
        roots = find_all_roots_poly(poly_normed, x_min=x_min, x_max=x_max, num_intervals=num_intervals, tol=1e-6, max_iter=80)
        if not roots:
            skipped_no_root += 1
            continue
        ...
        r1_list.append([float(r1)] if r1 is not None else [np.nan])
        r2_list.append([float(r2)] if r2 is not None else [np.nan])
...
def save_splits(
    ...
    for split_name, split_idx in splits.items():
        path = out_dir_p / f"taylor_deg{degree}_{split_name}.npz"
        payload = dict(
...
# ======================================
def main():
    cfg_path = os.environ.get("DATASET_CFG", "configs/dataset_interval_physchem_v4_deg25.yaml")
    cfg = load_cfg(cfg_path)

    degree = int(_get(cfg, "dataset.degree", 25))
    n_total = int(_get(cfg, "dataset.n_total", 1000000))
    seed = int(_get(cfg, "dataset.seed", 42))
    out_dir = str(_get(cfg, "dataset.out_dir", "./taylor_data_physchem_v4_interval"))
    ...
    print("[DONE] interval dataset generation finished (YAML-based, no-arg).")
